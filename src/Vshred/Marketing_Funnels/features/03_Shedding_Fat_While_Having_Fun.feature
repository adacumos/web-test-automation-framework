@smoke
Feature: Shedding Fat While Having Fun
    Background: Load test data
        Given The user loads the VS test data

    Scenario: Fat loss for extreme for men funnel
        Given The user visits the base VShred URL
        Then The User verifies the video on the homepage - shedding fat while having fun
