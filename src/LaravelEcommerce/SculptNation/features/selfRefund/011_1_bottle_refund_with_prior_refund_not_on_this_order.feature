@selfRefund
Feature: One Bottle Self Refund with Prior Refund Not On This Order
    ...
    Scenario: Self Refund With Prior Refund Not On This Order
        ...
        When The user creates a second one bottle order
        Given The user logs into the SculptNation account
        When The user settles the payment
        Then The user "creates" a self "refund" with "prior refund not on this order"
        Then The user verifies "one bottle" self refund with "prior refund not on this order" in the admin tool
