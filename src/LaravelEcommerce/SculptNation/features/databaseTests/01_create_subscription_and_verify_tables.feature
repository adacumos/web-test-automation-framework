@smoke @MD @database
Feature: Create a subscription order and verify database tables
    Background: Load test data
        Given The user loads the LE test data

    Scenario: Verify the data on Users, Orders, Payments and Subscriptions tables exists and correct when creating a subscription order
        ...
        And The user processes the payment using a "valid" credit card
        And The user verifies the created user on the Users table
        And The user verifies the created order on the Orders table
        And The user verifies the created subscription on the Subscriptions table
