const vsAcceleratorQuestionnairePageLocators = {
    ...
    timezone: () => cy.get('textarea[name=Time_Zone]'),
    startDate: () => cy.get('input[name=start_date]'),
    ...
    workoutPreference: (answer: string) =>
        cy.get(`input[name="Workout_preference"][value*="${answer}"]`).next(),
    ...
    phaseApproach: (answer: string) =>
        cy.get(`input[name=Phased_Approach][value=${answer}]`).next(),
...
export default vsAcceleratorQuestionnairePageLocators;
